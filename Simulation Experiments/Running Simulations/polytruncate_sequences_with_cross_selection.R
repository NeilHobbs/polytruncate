...
for(v in 7100:nrow(parameter.space.df)){
  ...
  sequence.list[[v]] = temp_2

  if(v %% 10 == 0){print(v)}
}


sequence.truncation.df = do.call(rbind, sequence.list)

final.sequence.df = cbind(parameter.space.df, sequence.truncation.df)


write.csv(final.sequence.df, ".//sequence.truncation.with.cross.selection.csv")
