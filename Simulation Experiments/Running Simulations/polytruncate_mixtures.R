...
for(v in 1:nrow(parameter.space.df)){
  ...
  peak.resistance = max(temp_intervention$resistance.intensity)

  average.resistance.score.1 = as.vector(temp_intervention%>%
                                           dplyr::filter(deployed.mixture.part.1 == insecticide.tracked)%>%
                                           summarise(mean(resistance.intensity)))

  average.resistance.score.2 = as.vector(temp_intervention%>%
                                           dplyr::filter(deployed.mixture.part.2 == insecticide.tracked)%>%
                                           summarise(mean(resistance.intensity)))


  temp_2 = data.frame(simulation.duration, average.resistance.score.1, average.resistance.score.2, peak.resistance)


  mixture.list[[v]] = temp_2

  if(v %% 10 == 0){print(v)}
}


mixture.truncation.df = do.call(rbind, mixture.list)

final.mixture.df = cbind(parameter.space.df, mixture.truncation.df)


write.csv(final.mixture.df, ".//mixture_truncation.csv")
