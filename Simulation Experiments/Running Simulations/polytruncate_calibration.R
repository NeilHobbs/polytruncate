...
for(v in 1:nrow(parameter.space.df)){
  ...
  calibration.list[[v]] = simulation.duration

  if(v %% 10 == 0){print(v)}
}


sim.duration = do.call(rbind, calibration.list)

df = cbind(parameter.space.df, sim.duration)
# write.csv(df, ".//polytruncate_calibration.csv")

calibration.df = read.csv(".//polytruncate_calibration.csv")
library(ggplot2)

df.1 = calibration.df|>
  dplyr::filter(sim.duration < 500)

ggplot(df.1, aes(x=sim.duration/10))+
  ...
  geom_vline(xintercept = 12,
             ...
             alpha = 0.7,
             linetype = "dashed")+
  geom_rect(data = subset(df.1, exposure.scaling.factor %in% 1 & standard.deviation %in% 20),
...
plot.b = ggplot(subset(calibration.df, exposure.scaling.factor %in% 1 & standard.deviation %in% 20), aes(x=sim.duration/10))+
  geom_histogram(binwidth = 1,
                 fill = "#99d8c9",
                 colour = "white")+
  geom_vline(xintercept = 8,
...
"
plot.a + plot.b +
  plot_layout(design = layout) + plot_annotation(title = "polytruncate")
