...
plot_resistance_to_bioassay_survival = function(maximum.bioassay.survival.proportion = 1, 
    ...
    dplyr::mutate(bioassay.survival.proportion = resistance_to_bioassay_survival(
      ...
      number.bioassays=number.bioassays)) ##plotting with sigma as 0.1 or 25 made no difference to plots
    ...
    ylab("Bioassay Survival Proportion to WHO Cylinder Test")+ #note: this label may need to be changed depending on what we calibrate against
    theme_classic()
}
